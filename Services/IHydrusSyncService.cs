...
public interface IHydrusSyncService
{
    ...
    /// </summary>
    /// <param name="comicTitle">Name of the title to extract (without the namespace prefix)</param>
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>Preparation data containing ordered pages, metadata, and chapter starts.</returns>
    Task<ComicImportPreparation> ExtractComicAsync(string comicTitle, CancellationToken cancellationToken = default);
    ...
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>Preparation data containing ordered pages, metadata, and chapter starts.</returns>
    Task<ComicImportPreparation> ExtractComicAsync(string comicTitle, HydrusSourceMapping? sourceMapping, CancellationToken cancellationToken = default);
    ...
    /// </summary>
    /// <param name="comicTitle">Name of the title to sync (without the namespace prefix)</param>
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>The synchronized series ID, or null if sync failed</returns>
    Task<int?> SyncComicAsync(string comicTitle, CancellationToken cancellationToken = default);
    ...
    /// </summary>
    /// <param name="comicId">The local cached series identifier.</param>
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>True when the series existed and was deleted; otherwise false.</returns>
    Task<bool> DeleteComicAsync(int comicId, CancellationToken cancellationToken = default);
